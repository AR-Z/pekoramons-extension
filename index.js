...
`;
  ...
  async function fetchLatestPlayerSnapshot(userId){
    ...
    const url = `https://pekoramons.xyz/api/player-history?playerId=${encodeURIComponent(userId)}`;
    try{
  ...
  async function ensureProfileValueBox(){
    try{
      ...
      if(existing){
        ...
        if(a){ a.href = `https://pekoramons.xyz/player/${encodeURIComponent(userId)}`; const span = a.querySelector('.pekora-value-num'); if(span) span.textContent = displayText; }
        if(usernameEl){
      ...
      a.href = `https://pekoramons.xyz/player/${encodeURIComponent(userId)}`;
